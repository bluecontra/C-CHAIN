...
if __name__ == '__main__':
    ...
    parser.add_argument('--alg', type=str, default='ppo_cchain', help='alg name')
    ...
    parser.add_argument('--target_p_rel_loss_scale', type=float, default=10000,
                        help='the target policy relative scale for chain')
    ...
    for cur_env in env_generator:
        ...
        while level_steps < args.level_steps:
            ...
            summary = agent.train_with_churn_reduction(optimizer=optimizer, p_reg_coef=chain_p_reg_coef)

            # NOTE - adjust reg coef according to the relative scale of losses
            running_p_loss_list.append(summary['Loss/pi'])
            running_v_loss_list.append(summary['Loss/v'])
            running_p_reg_loss_list.append(summary['Loss/p_reg_loss'])
            # NOTE
            if level_iters >= 20:
                if args.target_p_rel_loss_scale > 0:
                    running_p_loss = np.mean(np.abs(running_p_loss_list[-200:]))
                    running_p_reg_loss = np.mean(running_p_reg_loss_list[-200:])
                    chain_p_reg_coef = max(args.target_p_rel_loss_scale * running_p_loss / (running_p_reg_loss + 1e-8),
                                           1)
            ...
            if level_iters % args.log_freq == 0:
                ...
                wandb.log({'Stats/auto_p_reg_coef': chain_p_reg_coef},
                          step=global_step)
                wandb.log(episode_stats, step=global_step)
