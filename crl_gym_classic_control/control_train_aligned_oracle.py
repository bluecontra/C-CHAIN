...
from basic_utils import Episode, History, train_combined_networks_invest
...
if __name__ == '__main__':
    ...
    parser.add_argument('--alg', type=str, default='ppo_reset_oracle', help='algorithm name')
    ...
    for ite in range(max_iterations):
        ...
        if ite % args.level_switch == 0:
            ...
            level += 1
            # NOTE - reset the model as it learns each task independent
        ...
        train_stats_dict, history_policy_list, history_value_list = \
            train_combined_networks_invest(policy_model, value_model, combined_optimizer,
        ...
        wandb.log({'global_timestep': global_step_count,
                   ...
                   'stats/policy_loss': train_stats_dict['p_loss'], 'stats/value_loss': train_stats_dict['v_loss']},
                  step=ite + 1)
        history.free_memory()
